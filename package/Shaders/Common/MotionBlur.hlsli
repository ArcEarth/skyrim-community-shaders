...
namespace MotionBlur
{
	...
	{
		float4 cameraMovement = FrameBuffer::CameraPosAdjust[a_eyeIndex] - FrameBuffer::CameraPreviousPosAdjust[a_eyeIndex];
		float4 screenPosition = mul(FrameBuffer::CameraViewProjUnjittered[a_eyeIndex], a_wsPosition);
		float4 previousScreenPosition = mul(FrameBuffer::CameraPreviousViewProjUnjittered[a_eyeIndex], a_wsPosition + cameraMovement);
